...
enlist_by_income %>%
  summarise(
    ...
    avg_enlist = mean(enlist_per_10K),
...
ggplot(enlist_by_income, aes(x = avg_income, y = enlist_per_10K)) +
  ...
  labs(
    title = '2020 Military Enlistments by Average Income of Zip Code',
...
ggplot(enlist_by_income, aes(x = avg_income)) +
  ...
  labs(
    title = 'Distribution of Incomes in Military Enlistments by Zip Code 2010',
...
enlist_by_income <- enlist_by_income %>%
  mutate(
    ...
    log10_avg_income = log10(avg_income + 1)
  )
...
ggplot(enlist_by_income, aes(x = log10(avg_income), y = log10(enlist_per_10K))) +
  geom_point(alpha = 0.3, color = 'tomato') +
